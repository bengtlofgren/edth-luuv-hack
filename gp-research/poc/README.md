...
| `poc_07_dynamics_gp.py` | Control-conditioned dynamics GP (PILCO-style, Deisenroth & Rasmussen): regress acceleration on state + commanded thrust, a = f(v, u), and bridge an outage by rolling the model forward with the commands the vehicle still knows — attacks the maneuver-overconfidence result at its root (the maneuver is no longer unobserved). Single-gap demo on SOLAQUA; ensemble verdict in [`../eval/RESULTS.md`](../eval/RESULTS.md) | refs 01–04, impl 02 |
